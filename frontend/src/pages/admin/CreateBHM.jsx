...
const CreateBHM = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/admin/bhm', formData);
      navigate('/admin/bhm');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to create BHM');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
      <h1 className="text-2xl font-bold mb-6">Create New BHM</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Assign to RHM</label>
          <select name="rhmId" value={formData.rhmId} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md p-2">
            <option value="">Select RHM</option>
            {rhms.map(rhm => (
              <option key={rhm._id} value={rhm._id}>{rhm.name} ({rhm.armyno})</option>
            ))}
          </select>
        </div>
        <div className="flex justify-end space-x-3">
          <button type="button" onClick={() => navigate('/admin/bhm')} className="px-4 py-2 border border-gray-300 rounded-md">Cancel</button>
          <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
            {loading ? 'Creating...' : 'Create BHM'}
          </button>
        </div>
      </form>
    </div>
